...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  showToast('Будильник установлен на '+ target.toLocaleString(), 'success', 2500);
  scheduleCountdown();
}

// Функция отложить будильник на 5 минут
function snoozeFive(){
  if (!scheduledTarget && !alarmTimer) {
    showToast('Будильник не установлен', 'warn');
    return;
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Инициализация будильников и таймеров
  alarms = JSON.parse(localStorage.getItem('motiv_alarms') || '[]');
  timers = JSON.parse(localStorage.getItem('motiv_timers') || '[]');

  // Создание будильника по умолчанию, если их нет
  if (alarms.length === 0) {
    alarms.push({
      id: Date.now(),
      time: '07:00',
  ...
  // Создание таймера по умолчанию, если их нет
  if (timers.length === 0) {
    timers.push({
      id: Date.now(),
      duration: 300, // 5 минут
  ...
  document.getElementById('setWake').addEventListener('click', ()=>{
    ...
    if (!t){ showToast('Укажите время', 'warn'); return; }
    setWakeUp(t);
  });
  ...
  document.getElementById('snooze').addEventListener('click', ()=>{
    const minutes = Number(document.getElementById('snoozeSelect').value) || 5;
    // compute new target +minutes from now
    const now = new Date();
    const newTarget = new Date(now.getTime() + minutes*60*1000);
    const hh = String(newTarget.getHours()).padStart(2,'0');
  ...
  // Обработчики для будильников и таймеров
  document.addEventListener('click', (e) => {
    // Обработчики для будильников
    if (e.target.classList.contains('alarm-switch')) {
      const id = parseInt(e.target.dataset.id);
      const alarm = alarms.find(a => a.id === id);
      if (alarm) {
  ...
  // Заполнение статистики пропущенных дней
  const stats = getStats();
  const todayKey = new Date().toISOString().slice(0,10);
  // ensure only one miss per previous day: fill up last 7 days
  const now = new Date();
